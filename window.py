...
# https://blog.csdn.net/qq_44239466/article/details/90250938
...
from TencentTrans import tencentTranslate
from BaiDuTrans import baiduTranslate
from YoudaoTrans import connect
from AliTrans import alicloud
...
def Tips():
    tk.messagebox.showinfo("Tips", "PDF文件大小无上限，推荐5MB以下，过大影响翻译速度\n翻译速度受接口QPS、网络状况等多重影响，请耐心等待\n本软件仅用作毕业设计以及日常使用，请勿用于非法用途\n若接口方更改免费政策，可能导致软件部分功能无法使用")
...
def About():
    ...
    about_window.iconbitmap(".\\fan.ico")
    fan_gif = tk.PhotoImage(file=".\\fan.gif")
    ...
    ttk.Label(software_frame, image=fan_gif, compound='left').grid(row=0, rowspan=3, column=0)
    ttk.Label(software_frame, text="PDF Translate Version 1.0").grid(row=0, column=1, sticky = W)
    ttk.Label(software_frame, text="@Author    :   lijishi").grid(row=1, column=1, sticky = W)
    ttk.Label(software_frame, text="@EditTime  :   Feb 1,2020").grid(row=2, column=1, sticky = W)
    ...
    ttk.Label(copyright_frame, text = "Github @ PDF_Translate").grid(row=5, column=0)
...
def youdao(language):
    try:
        ...
        for index in range(total_lines):
            ...
            Is2language = int(double_language.get())
            return_text = connect(line, language, Is2language)
...
def tencent(language):
    try:
        ...
        for index in range(total_lines):
            ...
            time.sleep(0.2)
            Is2language = int(double_language.get())
            return_text = tencentTranslate(line, language, Is2language)
            if return_text[0:17] == 'TencentCloudError':
                tk.messagebox.showerror("TencentCloudError", return_text[17:])
                now_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
                scr.insert(END, now_time)
                scr.insert(END, ' 翻译失败\n')
                scr.insert(END, '\n')
                scr.insert(END, return_text[17:])
                ready_txt.close()
                os.remove('.\\pdf2txt_temp.txt')
                os.remove('.\\out_temp.txt')
                return
...
def baidu(language):
    try:
        ...
        for index in range(total_lines):
            ...
            Is2language = int(double_language.get())
            return_text = tencentTranslate(line, language, Is2language)
            if return_text[0:10] == 'BaiDuError':
                tk.messagebox.showerror("BaiDuError", return_text[10:])
                now_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
                scr.insert(END, now_time)
                scr.insert(END, ' 翻译失败\n')
                scr.insert(END, '\n')
                scr.insert(END, return_text[10:])
                ready_txt.close()
                os.remove('.\\pdf2txt_temp.txt')
                os.remove('.\\out_temp.txt')
                return
...
def ali(language):
    try:
        ...
        for index in range(total_lines):
            ...
            time.sleep(0.02)
            Is2language = int(double_language.get())
            return_text = alicloud(line, language, Is2language)
            if return_text[0:13] == 'AliCloudError':
                tk.messagebox.showerror("AliCloudError", return_text[13:])
                now_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
                scr.insert(END, now_time)
                scr.insert(END, ' 翻译失败\n')
                scr.insert(END, '\n')
                scr.insert(END, return_text[13:])
                ready_txt.close()
                os.remove('.\\pdf2txt_temp.txt')
                os.remove('.\\out_temp.txt')
                return
...
def Translate():
    ...
    if api_choose == 1:
        ...
        thread_Ali = MyThread(ali, args=(language_change,))
        thread_Ali.start()
...
# window layout
main_window.title("PDF Translate V1.0")
main_window.iconbitmap(".\\fan256.ico")
...
page = IntVar()
double_language = IntVar()
Radiobutton(page_frame, text = "全部翻译", variable = page, value = 1).grid(row = 2, column = 0, padx = 10, pady = 5)
ttk.Checkbutton(page_frame,text='双语输出',variable=double_language, onvalue=1, offvalue=0).grid(row = 2, column = 3, columnspan = 2, padx = 10, pady = 5)
...
apiChosen['values'] = ('接口选择', '有道翻译', '百度翻译', '腾讯翻译', '阿里翻译')
apiChosen.current(0)
file = tk.StringVar()
fileChosen = ttk.Combobox(outmode_frame, width=8, textvariable=file)
fileChosen.grid(row = 4, column = 5, padx = 5, pady = 6, sticky = W)
